...
function DeleteModal({ student, onClose, onConfirm, deleting }) {
  return (
    <div
    ...
    >
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-sm p-6">
        ...
        <h2 className="text-white font-semibold text-center mb-1">Delete Student</h2>
        <p className="text-zinc-400 text-sm text-center mb-6">
          Are you sure you want to delete{" "}
          <span className="text-white">{student.first_name} {student.last_name}</span>?
...
function EditModal({ student, onClose, onSaved }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/admin/students/${student.id}/update/`, {
        method: "PUT",
      ...
      if (!res.ok) {
        setError(Object.values(data).flat().join(" ") || "Failed to update student")
      } else {
        onSaved()
      }
    } catch {
    ...
    >
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-white font-semibold">Edit Student</h2>
            <p className="text-zinc-500 text-xs mt-0.5 font-mono">{student.username}</p>
          </div>
          <button
            onClick={onClose}
...
function DetailModal({ student, onClose, onEdit, onDelete }) {
  const rows = [
    { label: "Username",   value: student.username,   mono: true },
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-sm p-6">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-xs">{student.department}</p>
          </div>
        </div>
...
export default function AdminStudents() {
  ...
  const [selected, setSelected] = useState(null)   // student for detail
  const [editing, setEditing]   = useState(null)   // student for edit modal
  const [deleting, setDeleting] = useState(null)   // student for delete modal
  ...
  async function handleDelete() {
    ...
    try {
      const token = localStorage.getItem("access")
      const res = await fetch(`${API}/api/admin/students/${deleting.id}/delete/`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      })
      if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <nav className="border-b border-zinc-800 px-6 py-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-white font-medium text-sm">Students</h1>
          <p className="text-zinc-500 text-xs">{students.length} total</p>
        </div>
      </nav>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-white font-medium text-sm">All Students</h2>
            ...
            <div className="space-y-2">
              ...
              {filtered.map((student, index) => (
                ...
                >
                  ...
                  <span className="col-span-3 text-xs font-mono text-blue-400">{student.username}</span>
                  <span className="col-span-4 text-zinc-100 text-sm">
                    {student.first_name} {student.last_name}
                  </span>
                  <div className="col-span-4 flex items-center justify-between">
                    <span className="text-zinc-500 text-xs">{student.department}</span>
